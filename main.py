...
def handle_area(ctx, container, message):
    ...
    if any(x in ctx['upper'] for x in ["WAITING AREA", "HOLDING AREA", "ANCHORAGE AREA", "TEMPORARY STAY AREA"]):
        if len(ctx['coords']) >= 3:
...
def handle_trackline(ctx, container, message):
    ...
    TRACK_KEYWORDS = ["TRACKLINE", "JOINING", "PIPELINE", "CABLE", "CHANNEL"]
...
def handle_multipoint(ctx, container, message):
    ...
    if ctx['is_riglist']:
        return False
    if not is_multi_point_navarea(ctx['block']):
        return False
    style = get_point_style(ctx['block'])
    color = detect_color(ctx['block'])
    check_danger = detect_check_danger(ctx['block'])
...
def run_regression_tests():
    ...
    for tf in test_files:
        ...
        try:
            ...
            # Parse and process
            blocks = re.split(r'(?=NAVAREA\s+[A-ZIVXLC]+\s+\d+/\d+)', test_text, flags=re.IGNORECASE)
            navs = {}
            for block in blocks:
                block = block.strip()
                if not block:
