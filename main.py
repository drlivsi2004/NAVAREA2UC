...
def detect_arc_area(block):
    ...
    arc_phrase = re.search(
        r'ARC\s+OF\s+(\d+(?:\.\d+)?)\s*NM\s+RADIUS',
        block,
        re.IGNORECASE
    )
    joining_phrase = re.search(
        r'JOINING\s+POINT\s+([A-Z])\s+AND\s+([A-Z])',
        block,
        re.IGNORECASE
    )
    ...
    for msg in messages:
        ...
        if len(parts) == 1:
            ...
            for idx, part in enumerate(parts, start=1):
                ...
                print(f"     Part {idx} â {part_objects} objects, {part_vertices} vertices")
...
def export_furuno_modern(nav_id, container):
    ...
    xml.append(f'<userchart name="{xml_attr(f"NAVAREA {nav_id} IMPORT")}" description="" version="1.3">')
    ...
    if container.get('lines'):
        ...
        for line in container['lines']:
            ...
            xml.append('<position>')
            for idx, (lat, lon) in enumerate(line['coords'], start=1):
                xml.append(f'<vertex id="{idx}" latitude="{lat:.6f}" longitude="{lon:.6f}"/>')
            xml.append('</position>')
