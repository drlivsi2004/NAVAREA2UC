...
NAVAREA_PATTERN = re.compile(
    r"(?=NAVAREA\s+[A-ZIVXLC]+\s+\d+/\d+)", flags=re.IGNORECASE
)
...
def discover_source_files(root):
    return sorted(root.glob("NAV-*.txt"), key=lambda path: path.name)
...
def explicit_geometry_kinds(block):
    ...
    if len(route_points) >= 2 or LINE_EXPLICIT_PATTERN.search(block):
        kinds.append("line")
    if main.extract_circle_spec(block):
        kinds.append("circle")
    if AREA_EXPLICIT_PATTERN.search(block) or main.extract_area_group_sections(block):
        kinds.append("area")
    return kinds
...
def process_source_block(block, navarea_name, region):
    ...
    missing_components = [
        ...
        and not (kind == "area" and rejected_area_count)
    ]
...
def main_cli():
    ...
    parser.add_argument(
        ...
        help="Workspace root containing NAV-*.txt files",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        report = build_corpus_report(args.root)
