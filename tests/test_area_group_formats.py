...
def _build_container_and_message():
    container = main.create_container('NAVAREA_TEST')
    message = main.create_message('NAVAREA_TEST')
...
46-09.0N 031-18.7E
'''
    groups = main.extract_area_group_sections(block)
    assert len(groups) == 3
    assert [g[0] for g in groups] == ['A', 'B', 'C']
    assert [len(main.extract_coordinates(g[1])) for g in groups] == [5, 4, 5]
...
'''
    ...
    ctx = main.build_processing_context(block, 'NAVAREA III 92/22')
    matched = main.handle_area(ctx, container, message)
