...
- Geometry/classification: `V 502/26` has a source-order geometry defect with
  ...
  still needs authoritative/physical confirmation. `VII 217/2026` loses
  positions A–C and emits a false D–F line; `VIII 729/26` has an unresolved hazard-line-versus-points
  interpretation; `VIII 895/26` retains only the first of four reference
  coordinates; `III 122/26` currently loses the first Area vertex in the
  physical import and displays a triangle. The five repaired Areas above
  remain open only for physical import confirmation.
- `V 449/26`: blue Point/Label remains a manual design reference, not runtime
  output.
...
|---|---|---|---|---|---|
| ECDIS-001 | `attached_assets/image_1787929587528.jpg` (earlier artifact); `attached_assets/IMG_6713_1788093313089.jpeg` (supplied physical retest) | `tests/fixtures/navarea_iii_spain_122_26.txt`, `NAVAREA III 122/26` | The supplied retest shows a blue object with width 2 and density 25%, labelled `NAV III 122/26`. The Description begins with the expected `BLACK SEA` / `DRILLING OPERATIONS BY ABDULHAMID HAN RIG` context. | The source boundary is `A,B,C,D` where `A=43-05.03N 031-36.52E`, but the ECDIS table visibly contains `B,C,D,B`: the first source vertex `A` is missing and the last row duplicates `B`. The chart therefore displays a triangle, not the required four-sided Area. The frame also reports `There are duplicate points!`; this is consistent with the malformed imported vertex sequence. | Physical retest: Area styling and Description beginning observed, but geometry is rejected; the first source vertex is lost and the object must be rebuilt and retested |
...
| ECDIS-007 | `attached_assets/IMG_6691_1788015974418.jpeg` (manual User Chart edit) | `NAVAREA V - BRAZIL.txt`, `NAVAREA V 449/26` | A physical ECDIS User Chart demonstrates the proposed visual semantics for `V 449/26`: the preparatory offshore activity is represented as a blue Point/Label, visually associated with a confirmed blue platform diamond. | This is a manually edited visualization of the proposed design, not generated v1.3.0 output and not evidence of a hidden route or provider algorithm. The blue `V 515/26` label visible in the same screenshot was accidental and is not part of the intended semantic rule. | Design reference recorded for `V 449/26`; `V 515/26` remains normal informational |
...
| ECDIS-011 | `attached_assets/IMG_6702_1788020649477.jpeg`, `IMG_6701_1788020649477.jpeg`, `IMG_6700_1788020649477.jpeg` (system-generated ECDIS artifacts) | `NAVAREA VIII - INDIA.txt`, `NAVAREA VIII 895/26`, `VIII 806/26`, and `VIII 789/26` | `VIII 895/26` displays an Orange/Point at 22-24.850 N 067-30.660 E; `VIII 806/26` displays an Orange/Point at 15-21.160 N 073-45.770 E; `VIII 789/26` displays an Orange/Point at 19-11.500 N 072-46.400 E. All three coordinates match the source and current parser output. | `VIII 895/26` publishes four hydrographic-survey vicinity coordinates, but the artifact and current parser place only the first coordinate in the Point object and retain the other three only in Description. No line is inferred; whether all reference positions should become independent points remains open. | Physical artifacts confirm single-point presentation; `VIII 895/26` reference-coordinate retention remains open |

| ECDIS-012 | `attached_assets/IMG_6704_1788054988024.jpeg` (system-generated ECDIS artifact) | `NAVAREA IX - PAKISTAN.txt`, `NAVAREA IX 58/2023` | The physical ECDIS shows an Orange `NAV` object with width 3, a three-vertex table, and the source positions: `26-35.496N 052-01.986E`, `26-38.808N 051-53.496E`, and `26-40.375N 051-53.896E`. The visible Description retains the message number, timestamp, `CENTRAL GULF`, and `QATAR` context. | The source describes 1500-meter safety zones established around SPM2, SPM3, and SPM4; it does not explicitly state a `ROUTE`, `TRACKLINE`, or connecting segment. The current artifact nevertheless presents the three positions as one Line. Whether this should become separate explicit circles, points, or another reviewed representation remains open. | Physical artifact confirms current coordinates, description context, and Line presentation; geometry semantics open |
...
| ECDIS-014 | `attached_assets/IMG_6706_1788055436061.jpeg` (system-generated ECDIS artifact) | `NAVAREA IX - PAKISTAN.txt`, `NAVAREA IX 7/2026`, Section 1 | The physical ECDIS shows an Orange `NAV` object with width 3 and two visible vertices matching the source: `24-12.360N 052-38.160E` and `24-11.870N 052-37.860E`. The visible Description retains the message number, timestamp, `SOUTHERN GULF`, and `UAE` context. The release report records two coordinates, one Label, and one Line. | The source says a disposal pipeline is afloat with two yellow mark buoys deployed in positions A and B; it does not explicitly define a `ROUTE`, `TRACKLINE`, or connecting segment. The current artifact presents the two buoy positions as one Line. Whether they should be independent Point/AtoN objects, and how their yellow-mark semantics should be represented, remains open. | Physical artifact confirms current two-coordinate and Line presentation; independent buoy classification and false-line question open |
...
| ECDIS-017 | `attached_assets/IMG_6709_1788057916188.jpeg` (system-generated ECDIS artifact) | `NAVAREA IX - PAKISTAN.txt`, `NAVAREA IX 102/2022`, Section 3 | The physical ECDIS shows the selected object as Orange `Style: Point`, with label `NAV IX 102/2022` and one coordinate: `24°34.680'N 067°04.070'E`. This matches the source's `NEW BOUY NO-3 ... FL(3)G 7S ... LAYED IN POSITION 24 34.68N 067 04.07E`. The visible Description begins with the NAVAREA header, timestamp, notice number, Pakistan, and Port Muhammad Bin Qasim context; the field has a scrollbar and the frame does not show its complete contents. | The Point geometry and source coordinate are confirmed, but the object does not match the published buoy visual language: an active/established navigation buoy is expected to be Yellow, Non-danger, and triangle-marked, while this artifact is a generic Orange Point. The source spells `BOUY`, while the current buoy classifier recognizes `BUOY`; the release corpus therefore records `handle_single_point` rather than `handle_buoy_semantics`. This marking/classification mismatch is separate from geometry and Description. Description completeness is not assessed from this frame because only the upper portion of the scrollable field is visible. | Point geometry and source coordinate confirmed; documented buoy marking not confirmed; Description full-content review remains unassessed |
...
`NAVAREA VIII 729/26` physically reproduces the current Orange `NAV` line,
but the source says lesser depths are reported in a channel in the vicinity of
two positions; it does not explicitly describe a route, trackline, or segment
between the points. The artifact therefore confirms current behavior, not
that the line is semantically a safe navigable route. Whether to retain a
hazard segment or emit independent points remains open.
...
| `NAVAREA III 122/26` | Export each boundary vertex once and let ECDIS close the Area without a repeated closing row; verify the cleaned Description on ECDIS. | Area vertex order/count, four-sided geometry, operation text, and cancellation removal |
